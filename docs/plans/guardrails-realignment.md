...
**Current working-tree status (2026-08-27):** Phases 0–5 are implemented and locally verified as recorded in §9, including evidence integrity, authentication and registry authority, governed credit decisions, model governance, synthetic testing, native adapters, formal-verifier repairs, sandbox obligations, and declarative guardrail composition. Live Docker execution remains a required CI-only gate; the integration job now fails before collection unless both the Docker SDK and daemon are available. The implementation remains uncommitted and is not a release artifact until it is reviewed and committed.
