...
### `dataset-curator`
**Purpose:** Generate, document, and publish synthetic benchmark datasets.

**Owns:**
- `scripts/generate_datasets.py` — deterministic generator driver for all synthetic datasets
- `datasets/README.md` — schema documentation, generation methodology, usage examples
- Reproducible dataset specs (regenerated on demand, not shipped as binaries):
  - `applications` → `synthetic_credit_applications_v1/` — consumer loan applications, ~8% default
  - `performance`  → `synthetic_loan_performance_v1/` — higher default rate for vintage analysis
  - `compliance_eval` → `credit_agent_compliance_eval_v1/` — edge-case scenarios for policy testing

**Key constraints:**
- No real customer data — all synthetic, generated by `SyntheticCreditGenerator` (statistical) or the optional WGAN-GP in `domains/finance/synthetic/`
- Datasets are regenerated on demand via `python scripts/generate_datasets.py`; large binaries are **not** committed to the repo
- Dataset `metadata.json` captures size, seed, target default rate, and output formats for reproducibility
...
- Publish datasets alongside the package release
...
7. **`dataset-curator` is fully parallel** — synthetic data generation has no dependency on the governance runtime.
